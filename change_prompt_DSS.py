...
def initialize_llm(use_deepseek: bool = True) -> Any:
    """初始化语言模型"""
    if use_deepseek:
        config = DEEPSEEK_CONFIG
        api_key = config['api_key']
        base_url = config['base_url']
        return ChatDeepSeek(model="deepseek-chat", api_key=api_key, base_url=base_url, temperature=0)
    else:
        # 可选择使用OpenAI
        ...
        return ChatOpenAI(temperature=0, model_name="gpt-4.1-2025-04-14")


def read_outline_file(subject: str) -> str:
    """读取主题大纲文件"""
    outline_file = f"./outline_{subject}.md"
    try:
        with open(outline_file, "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        raise FileNotFoundError(f"找不到大纲文件: {outline_file}")
    except Exception as e:
        raise Exception(f"读取大纲文件时出错: {str(e)}")


def get_file_paths(subject: str) -> Dict[str, str]:
    """获取各种文件路径"""
    return {
        "subject_template_file": f"./template/{subject}.md",
        "subject_kg_prompt_file": f"./graph_utils/{subject}_kg_prompt.md",
        "prompts_config_file": f"./graph_utils/chatgpt/config/prompts_config_{subject}.json"
    }


def get_ft_template() -> str:
    """获取费托合成模板"""
    return """# FT Experiment Template
...
def get_allowed_nodes_and_rels() -> Dict[str, list]:
    """获取允许的节点和关系类型"""
    ...
    allowed_nodes = ["description",
                ...
                "Documents"]
    return {"allowed_rels": allowed_rels, "allowed_nodes": allowed_nodes}


def get_kg_example() -> str:
    """获取知识图谱示例"""
    return """# Knowledge Graph Instructions for GPT-4 (Fischer-Tropsch Synthesis Edition)
...
def generate_prompts_for_subject(subject: str, use_deepseek: bool = True) -> Dict[str, Any]:
    """
    为指定主题生成提示词
    
    Args:
        subject: 主题名称，如 "Duplex Stainless Steel"
        use_deepseek: 是否使用DeepSeek模型，默认为True
        
    Returns:
        包含生成结果的字典
        
    Raises:
        FileNotFoundError: 当找不到大纲文件时
        Exception: 其他错误
    """
    print(f"\n{'='*60}")
    print(f"🎯 开始为主题 '{subject}' 生成提示词")
    print(f"{'='*60}")
    
    try:
        # 初始化模型和获取文件路径
        print(f"📋 步骤 1/10: 初始化配置...")
        print(f"   - 使用模型: {'DeepSeek' if use_deepseek else 'OpenAI'}")
        graphllm = initialize_llm(use_deepseek)
        print(f"   ✅ 模型初始化完成")
        
        print(f"📖 步骤 2/10: 读取大纲文件...")
        outline = read_outline_file(subject)
        print(f"   ✅ 大纲文件读取成功 (长度: {len(outline)} 字符)")
        
        print(f"📂 步骤 3/10: 获取文件路径...")
        file_paths = get_file_paths(subject)
        for key, path in file_paths.items():
            print(f"   - {key}: {path}")
        print(f"   ✅ 文件路径获取完成")
        
        print(f"📝 步骤 4/10: 加载模板和示例...")
        template = get_ft_template()
        nodes_rels = get_allowed_nodes_and_rels()
        kg_example = get_kg_example()
        print(f"   ✅ 模板和示例加载完成")
        
        # 1. 生成新的大纲
        print(f"\n🔄 步骤 5/10: 生成新的大纲...")
        print(f"   调用 LLM 进行大纲转换...")
        new_outlines = graphllm.invoke(f"""**Formal Transformation Prompt**

Transform a hierarchical outline containing main points and subpoints into a rewritten version where each subpoint is expanded into a full academic-style explanatory entry. For each group of subpoints under a main heading, merge them into a single continuous description that specifies what information should be extracted, how it should be distinguished, and what examples may apply. The rewritten output must:

* Use formal scholarly English with clear, directive phrasing (e.g., *Extract… Record… Document…*).
* Avoid preserving sub-subsections or bullet points; instead, produce integrated explanatory paragraphs under each main heading.
* Maintain the same hierarchical structure for the main headings only.
* Ensure the style is consistent, concise, and suitable for use as a standardized extraction guideline.
{outline}""").content

        print(f"   ✅ 大纲转换完成 (长度: {len(new_outlines)} 字符)")
        
        # 2. 生成新的主题模板
        print(f"\n🔄 步骤 6/10: 生成主题模板...")
        print(f"   调用 LLM 生成实验提取模板...")
        new_subject_template = graphllm.invoke(f"""
# Prompt for Constructing a Domain-Specific Experimental Template

**You are an expert in scientific information extraction and template design. Your task is to generate a detailed, structured experimental extraction template for academic papers, strictly following the provided outline.**

### Instructions

1. **Strictly follow the provided outline as the only structural framework.**
    - Every section, subsection, and bullet point in the outline must be reflected in the output template.
    - Do **not** add, remove, or merge any sections or subsections beyond what is present in the outline.
    - ✅ The **top-level headings (# ...)** must correspond **only** to the main titles in the outline.
    - ✅ The **second-level headings (## ...)** must correspond **only** to the main subsections in the outline, and must include numbering (e.g., `## 1. ...`, `## 2. ...`).
    - ✅ Subsections below that (e.g., 1.1, 1.2 …) must be written as **bold numbered list items** (e.g., `- **1.1 ...**`), **not as headings**.

2. **Formatting Rules**
    - Use Markdown for all output.
    - Only use `#` (top-level) and `##` (second-level) headings. No `###` or deeper levels in the template.
    - ✅ Do not place any descriptive text or instructions directly under a top-level heading (`# ...`). The first content under each `# ...` must be its first `## ...`.
    - For each `## ...` section:
      - Begin with an **Instruction** paragraph.
      - Then, list all required subsections as bold numbered items with detailed extraction rules.
      - Conclude with a **unified Example Output Format** block (in Markdown).
    - Example output formats may include deeper heading levels (`###`, `####`) **inside the code block only**, never in the main template structure.

3. **Reference Style and Granularity**
    - Match the style of the provided reference template, but apply it only to the given outline.
    - Do not copy irrelevant sections or terminology.
    - ✅ Avoid inserting explicit subject-matter examples; instructions must remain abstract and reusable.

4. **No extraneous content**
    - The output must only contain sections that are explicitly present in the provided outline.
    - No additional commentary or explanations outside the structure.
...
- A complete experimental extraction template matching the subject and outline.  
- ✅ Structure must follow this hierarchy:
```

# {{Main Title from new\_outlines}}

## 1. {{First major section from new\_outlines}}

* **1.1 …**
* **1.2 …**

## 2. {{Second major section from new\_outlines}}
...
- Each `## ...` section must contain:
  1. `**Instruction:**` paragraph  
  2. Bold numbered list items  
  3. `**Example Output Format:**` code block
```

""").content
        
        print(f"   ✅ 主题模板生成完成 (长度: {len(new_subject_template)} 字符)")
...
## Reference Examples
Node examples: {", ".join(nodes_rels["allowed_nodes"])}
Relationship examples: {", ".join(nodes_rels["allowed_rels"])}
...
4. The node list and relationship list are completely separated and must not be presented in a mixed manner or nested within each other.""").content
        ...
        print(f"   ✅ 节点和关系生成完成 (长度: {len(new_subject_nodes_rels)} 字符)")
        
        # 4. 生成知识图谱提示词
        print(f"\n🔄 步骤 8/10: 生成知识图谱提示词...")
        print(f"   调用 LLM 生成知识图谱指令...")
...
2. Reference Example: {kg_example} (the "Fischer-Tropsch Synthesis Knowledge Graph Instruction" as a structural template)  
...
Please combine {subject}, {kg_example}, and {new_subject_nodes_rels} to generate the new subject knowledge graph instruction in accordance with the above requirements.""").content
        
        print(f"   ✅ 知识图谱提示词生成完成 (长度: {len(new_subject_kg_prompt)} 字符)")
        ...
        for index, test_split in enumerate(text_splitter_markdown.split_text(new_subject_template)):
            try:
                sections_processed += 1
                if 'Header 2' in test_split.metadata:
                    print(f"      处理章节 {sections_processed}: {test_split.metadata['Header 2']}")
                count += 1
                new_subject_question_example = graphllm.invoke(f"""
You are an academic question generator. 
Your input is a section of an extraction template. 
Your task is to produce a short guiding instruction (2–4 sentences) that explains the goal of the extraction in a clear and scholarly way, without copying the template verbatim. 
Then, based on the given content, generate exactly 5 high-quality questions that start with "What" or "Which". 
The questions must be precise, unambiguous, and strongly aligned with the key extraction requirements in the content. 
Avoid redundancy, but ensure coverage of the main subsections and details mentioned.

Input:
{test_split.page_content}

Output:
- A rewritten guiding instruction paragraph tailored to the given content.
- A numbered list of 5 concise and content-specific questions.

""").content
                APPLICATION_PROMPTS["decompose_prompts"][str(count)] = new_subject_question_example
            except Exception as e:
                print(f"      ⚠️ 跳过章节 {sections_processed}: {str(e)}")
                continue
        
        print(f"   ✅ 分解提示词生成完成 (共处理 {count} 个章节)")
...
Please generate the corresponding abstract extraction prompt based on the above rules and in combination with the new subject {subject}.""").content
        ...
        print(f"   ✅ 摘要提示词生成完成 (长度: {len(new_subject_summary)} 字符)")
        ...
        # 9. 保存文件
        print(f"   保存模板文件: {file_paths['subject_template_file']}")
        with open(file_paths["subject_template_file"], "w", encoding="utf-8") as f:
            f.write(new_subject_template)
        print(f"   ✅ 模板文件保存成功")
        
        print(f"   保存知识图谱提示词: {file_paths['subject_kg_prompt_file']}")
        with open(file_paths["subject_kg_prompt_file"], "w", encoding="utf-8") as f:
            f.write(new_subject_kg_prompt)
        print(f"   ✅ 知识图谱提示词保存成功")
        
        print(f"   保存配置文件: {file_paths['prompts_config_file']}")
        APPLICATION_PROMPTS_FINAL = {"application_prompts": APPLICATION_PROMPTS}
        with open(file_paths["prompts_config_file"], "w", encoding="utf-8") as f:
            json.dump(APPLICATION_PROMPTS_FINAL, f, ensure_ascii=False, indent=2)
        print(f"   ✅ 配置文件保存成功")
        ...
        return {
            ...
            "subject": subject,
            "files_generated": {
                "template_file": file_paths["subject_template_file"],
                "kg_prompt_file": file_paths["subject_kg_prompt_file"],
                "prompts_config_file": file_paths["prompts_config_file"]
            },
            "generated_content": {
                "new_outlines": new_outlines,
                "new_subject_template": new_subject_template,
                "new_subject_nodes_rels": new_subject_nodes_rels,
                "new_subject_kg_prompt": new_subject_kg_prompt,
                "new_subject_summary": new_subject_summary
            }
        }
...
if __name__ == "__main__":
    ...
    result = generate_prompts_for_subject(subject)
    
    if result["success"]:
        print(f"成功为主题 '{subject}' 生成提示词")
        print("生成的文件:")
...
# FastAPI兼容的包装函数
def generate_prompts_api(request: PromptGenerationRequest) -> PromptGenerationResponse:
    """
    FastAPI兼容的提示词生成函数
    
    Args:
        request: 包含subject和use_deepseek参数的请求对象
        
    Returns:
        PromptGenerationResponse对象，包含生成结果或错误信息
    """
    result = generate_prompts_for_subject(request.subject, request.use_deepseek)
    return PromptGenerationResponse(**result)


# 简化的FastAPI兼容函数（直接接受参数）
def generate_prompts_simple(subject: str, use_deepseek: bool = True) -> Dict[str, Any]:
    """
    简化的提示词生成函数，适合FastAPI直接调用
    
    Args:
        subject: 主题名称
        use_deepseek: 是否使用DeepSeek模型
        
    Returns:
        包含生成结果的字典
    """
    return generate_prompts_for_subject(subject, use_deepseek)
