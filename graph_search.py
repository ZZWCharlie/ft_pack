...
def main(pdf_id,result_path,type_name):
    # Initialize logging
    init_logging()
    ...
    md_text = ""
    base_path = "./origin_paper/"+type_name+"_paper/"
    paths = []
    for i in range(1, 126):
        paths.append([base_path+str(i)+".pdf",base_path+str(i+1)+"-si.pdf"])
    ...
    save_dir = os.path.join(save_path,"Paper_"+str(pdf_id)+".md")
...
if __name__ == "__main__":
    type_name = "MoF"
    result_path = "./papersavings/"
    for pdf_id in range(1,126):
        if os.path.exists(result_path+type_name+"/Paper_"+str(pdf_id)+".md"):
            print(f"Paper_{pdf_id} already exists, skipping...")
            continue
        print(f"Processing Paper_{pdf_id}...")
        main(pdf_id,result_path,type_name)