...
def main(pdf_id):
    ...
    def process_section(args):
        ...
        chunk_results = {}
        for args in tqdm(chunk_args, desc=f"Processing section {template_num+1} QA", position=0, leave=False):
            chunk_start, chunk_result = process_qa_chunk(args)
    ...
    for task in tqdm(tasks, desc="Sequential Section Processing", position=0, leave=True):
