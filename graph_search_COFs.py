...
def group_pdf_files_with_si(subfolder_path):
    ...
    # subfolder_path = './origin_paper/COF-20250802/JACS'
    all_files = [f for f in os.listdir(subfolder_path) if f.endswith('.pdf')]
    # 分离主文件和SI文件
    main_files = []
    si_files = {}  # {main_name: [si_files]}
    
    for file in all_files:
        if '_si' in file:
            # 这是补充材料文件
            # 提取主文件名：去掉_si.pdf或_si_数字.pdf
            main_name = re.sub(r'_si(_\d+)?\.pdf$', '.pdf', file)
            if main_name not in si_files:
                si_files[main_name] = []
            si_files[main_name].append(file)
        else:
            # 这是主文件
    ...
    for main_file in main_files:
        ...
        if main_file in si_files:
            si_paths = [os.path.join(subfolder_path, si_file) for si_file in si_files[main_file]]
            # 按文件名排序，确保处理顺序一致
            si_paths.sort()
    ...
    # 处理没有对应主文件的SI文件
    for main_name, si_file_list in si_files.items():
        if main_name not in main_files:
...
def process_cof_folder(base_folder, result_path, type_name):
    """
    处理COF文件夹，为每个主文件及其SI文件生成独立的markdown文件
    """
    ...
    for subfolder in subfolders:
        ...
        # import pdb;pdb.set_trace()
        # 获取并分组PDF文件
        pdf_groups = group_pdf_files_with_si(subfolder_path)
        
        if not pdf_groups:
            print(f"No PDF files found in {subfolder_path}")
            continue
        
        print(f"Found {len(pdf_groups)} PDF groups in {subfolder}")
...
if __name__ == "__main__":
    ...
    base_folder = "/home/user/shizhou/Agent/ft_pack/origin_paper/COF-20250802"
    
    # 移除调试代码
    # import pdb;pdb.set_trace()
    
    # 处理整个COF文件夹
    process_cof_folder(base_folder, result_path, type_name)